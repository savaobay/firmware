...
enum ConfigError section_pos(struct IniConfig *ini, const char *section, int *start_pos, int *end_pos)
{
    for (unsigned int i = 0; i < MAX_SECTIONS; ++i)
    {
        if (ini->sections[i].pos > 0 && strcasecmp(ini->sections[i].name, section) == 0)
        {
...
enum ConfigError parse_enum(struct IniConfig *ini, const char *section, const char *param_name, void *enum_value,
                            const char *possible_values[], const int possible_values_count,
                            const int possible_values_offset)
{
    char param_value[64];
...
enum ConfigError parse_bool(struct IniConfig *ini, const char *section, const char *param_name, bool *bool_value)
{
    ...
    int val;
    enum ConfigError err = parse_enum(ini, section, param_name, (void *)&val, possible_values, count, 0);
    if (err != CONFIG_OK)
        return err;
    if (val % 2)
    {
...
enum ConfigError parse_int(struct IniConfig *ini, const char *section, const char *param_name, const int min,
                           const int max, int *int_value)
{
    char param_value[64];
    enum ConfigError err = parse_param_value(ini, section, param_name, param_value);
    if (err != CONFIG_OK)
...
enum ConfigError parse_array(struct IniConfig *ini, const char *section, const char *param_name, int *array,
...
enum ConfigError parse_uint64(struct IniConfig *ini, const char *section, const char *param_name, const uint64_t min,
...
{
    uint64_t val = 0;
    enum ConfigError err = parse_uint64(ini, section, param_name, min, max, &val);
    if (err != CONFIG_OK)
        return err;
    *value = val;
    return CONFIG_OK;
